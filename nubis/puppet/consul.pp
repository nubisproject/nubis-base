...
staging::file { 'envconsul.tar.gz':
  source => "https://www.github.com/hashicorp/envconsul/releases/download/v0.6.0/envconsul_0.6.0_linux_amd64.tar.gz",
} ->
staging::extract { 'envconsul.tar.gz':
  strip   => 0,
  target  => "/opt",
  creates => "/opt/envconsul_0.6.0_linux_amd64",
} ->
file { "/opt/hashicorp/envconsul_0.6.0_linux_amd64/envconsul":
  owner =>  0,
  group =>  0,
  mode  => '0555',
} ->
file { "/usr/local/bin/envconsul":
  ...
  target => '/opt/envconsul_0.6.0_linux_amd64/envconsul',
}
